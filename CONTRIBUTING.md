...
| `npm run dist:mac` | Build both, then `electron-builder --mac --x64 --arm64` (DMG + zip) — must run on macOS (`hdiutil`); CI job `build-macos` covers it |
...
> **macOS packaging note:** DMG creation requires `hdiutil`, so `npm run dist:mac`
> must run on a Mac (or the `build-macos` CI job, which runs on `macos-latest`
> and uploads the `.dmg` artifacts). The `build/icon.icns` icon is generated by
> `npm run icons` alongside `icon.png` / `icon.ico`.
